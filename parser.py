...
Construction_object = tuple[Code, Title] # Stavebný objekt
...
def get_person_type(person: OrderedDict[str, Any]) -> str:
    ...
    return get_item_code(person.get("PersonType", {}))
...
def create_address(physical_address: OrderedDict[str, Any]) -> Address:
    ...
    if city == "":
        city = get_item_name(physical_address.get("Municipality", {}))
        

    country: str = get_item_name(physical_address.get("Country", {}))
    county: str = get_item_name(physical_address.get("County", {}))
...
def create_physical_person(person_data: OrderedDict[str, Any]) -> PhysicalPerson:
    ...
    address: Address = create_address(person_data.get("PhysicalAddress", {}))

    phone = get_phone(person_data.get("TelephoneAddress", {}))
    mail = get_mail(person_data.get("ElectronicAddress", {}))
...
def create_corporate(person_data: OrderedDict[str, Any]) -> Corporate:
    ...
    physical_person = corporate_body.get("Stakeholder", {}).get("PersonData", {}).get("PhysicalPerson", {})
    ...
    ICO = get_ICO(person_data.get("ID", {}))
...
def create_person(person: OrderedDict[str, Any], person_type: str) -> Person:
    person_data: OrderedDict[str, Any] = person.get("PersonData", {})
...
def create_parcels(parcels: OrderedDict[str, Any]) -> Construction_parcels:
    ...
    parcels = ensure_list(parcels.get("ProjectParcel", {}))
...
def create_objects(facility: OrderedDict[str, Any]) -> list[Construction_object]:
    objects = []
    facility_parts = ensure_list(facility.get("FacilityParts", {}))

    for facility_part in facility_parts:
        facility_part_SOs = ensure_list(facility_part.get("FacilityPartSO", {}))
        for facility_part_SO in facility_part_SOs:
            objects.append( ( facility_part_SO.get("FacilityPartCode", "") or "", facility_part_SO.get("FacilityPartTitle", "") or "" ) )
...
def create_project(project: OrderedDict[str, Any]) -> Project:
    ...
    address = create_address(project.get("ProjectAddress", {}).get("PersonData", {}).get("PhysicalAddress", {}))
    parcels = create_parcels(project.get("ConstructionParcels", {}))
    objects = create_objects(project.get("Facilities", {}).get("Facility", {}))
    return Project(id, title, address, parcels, objects)
